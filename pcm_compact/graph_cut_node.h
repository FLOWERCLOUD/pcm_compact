...
struct GraphCutNode{
	...
	std::map<IndexType,IndexType> cor_frame_index;//save correspondence<frame,vertex_Id> 
...
public:
	...
	void constructBoxDis(std::unordered_map<IndexType, std::set<IndexType>>& label_set);
	void calculateNorm();
	//
	std::unordered_map<IndexType, std::set<IndexType>> label_bucket;//Index by frame and label
	std::unordered_map<IndexType, PCABox*> box_bucket;		//Index by frame and label
	std::unordered_map< IndexType, GraphCutNode* > node_map;
	std::vector<GraphCutNode*> node_vec;

public:
	std::map<IndexType,IndexType> boxId;//index by frame and label,out the id of  the box in map
	MatrixXX m_boxDis;
